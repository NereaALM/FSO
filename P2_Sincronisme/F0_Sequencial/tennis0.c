...
void carrega_parametres(const char *nom_fit)
{
	...
	if ((nFil_taulell < MInFil_taulell) || (nFil_taulell > MAX_FIL) ||
	(nCol_taulell < MInCol_taulell) || (nCol_taulell > MAX_COL) || (mida_porteria < 0) ||
	 (mida_porteria > nFil_taulell-3) || (long_pal < MIN_PAL) || (long_pal > nFil_taulell-3))
	{
	fprintf(stderr,"Error: dimensions del camp de joc incorrectes:\n");
	fprintf(stderr,"\t%d =< nFil_taulell (%d) =< %d\n",MInFil_taulell,nFil_taulell,MAX_FIL);
	fprintf(stderr,"\t%d =< nCol_taulell (%d) =< %d\n",MInCol_taulell,nCol_taulell,MAX_COL);
...
int inicialitza_joc(void)
{
	...
	fil_pal_usu = nFil_taulell/2; col_pal_usu = 3;		// inicialitzar pos. paletes 
	if (fil_pal_usu+long_pal >= nFil_taulell-3) fil_pal_usu = 1;
	...
	fil_pilota_R = fil_pilota; col_pilota_R = col_pilota;	// fixar valor real posicio pilota 
...
int moure_pilota(void)
{
	...
	{		v_fil_pilota_R = -v_fil_pilota_R; v_col_pilota_R = -v_col_pilota_R;	// canvia velocitats 
	...
	fil_pilota_R += v_fil_pilota_R; col_pilota_R += v_col_pilota_R;
	fil_pilota = f_h; col_pilota = c_h;		// actualitza posicio actual 
	...
	else { fil_pilota_R += v_fil_pilota_R; col_pilota_R += v_col_pilota_R; }
	return(result);
}
...
void mou_paleta_ordinador(void)
{
 ...
 // char rh,rv,rd; 
	...
	if (win_quincar(f_h+long_pal-1,col_pal_maq) == ' ')	 // si no hi ha obstacle 
	{
		...
		pVertical_pal_maq += v_pal_maq; fil_pal_maq = pVertical_pal_maq;		// actualitza posicio 
	...
	else pVertical_pal_maq += v_pal_maq; 	// actualitza posicio vertical real de la paleta 
}
